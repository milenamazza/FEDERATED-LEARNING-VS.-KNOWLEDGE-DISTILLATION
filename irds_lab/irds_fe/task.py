...
class CI_VAE_NUMERICAL(nn.Module):
    def __init__(self, input_size, hidden_size, latent_dim=7, num_classes=4, device=torch.device("mps" if torch.backends.mps.is_available() else "cpu")):
...
def load_data(partition_id: int, num_partitions: int, batch_size: int = 32) -> Tuple[DataLoader, DataLoader]:
    """Load partitioned data from a subject-based CSV file for federated learning."""
    global fds
    ...
    train_data = torch.cat((train_data, validation_data), 0)
    train_labels = np.append(train_labels, validation_labels)
   
    train_loader = torch.utils.data.DataLoader(
        torch.utils.data.TensorDataset(train_data, torch.tensor(train_labels, dtype=torch.float32)),
        batch_size=batch_size, shuffle=True
    )
    val_loader = DataLoader(
        torch.utils.data.TensorDataset(test_data, torch.tensor(test_labels, dtype=torch.float32)),
...
class ImprovedCI_VAE_NUMERICAL(nn.Module):
    def __init__(self, input_size=1280, hidden_size=512, latent_dim=7, num_classes=4, 
