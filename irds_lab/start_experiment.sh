...
BATCH_SIZES=("128")  # Make sure this matches what you want to test
GAMMAS=("0.1")
STEPS=("20")
FRACTIONS=("1.0")

# Available strategies
STRATEGIES=("fedadagrad")
...
ETA_ADAGRAD_VALUES=("0.001" "0.01" "0.1")
                                                            ...
                                                            for eta in "${ETA_ADAM_VALUES[@]}"; do
                                                                for beta_1 in "${BETA_1_VALUES[@]}"; do
                                                                    for beta_2 in "${BETA_2_VALUES[@]}"; do
                                                                        ...
                                                                        if is_combination_done "$supernodes" "$latent_dim" "$hidden_dim" "$rounds" "$epochs" "$batch" "$lr" "$gamma" "$step" "$fraction" "$strategy" "$eta" "$beta_1" "$beta_2"; then
                                                                            echo "⏩  Skipping already completed combination"
                                                                            continue
                                                                        fi
