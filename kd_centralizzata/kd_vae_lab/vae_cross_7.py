"""
Knowledge Distillation System with Hyperparameter Grid Search
Enhanced with best model selection, continuous result saving, and comprehensive cross-client logging
"""
...
# =============================================================================
# STUDENT MODEL - Lighter version of ImprovedCI_VAE_NUMERICAL
# =============================================================================

class StudentCI_VAE_NUMERICAL(nn.Module):
    """Lighter version of CI-VAE for student model"""
    
    def __init__(self, input_size=1280, hidden_size=256, latent_dim=7, num_classes=4, 
                 device=torch.device("cuda" if torch.cuda.is_available() else "cpu")):
        super(StudentCI_VAE_NUMERICAL, self).__init__()
        ...
        # Variational layers
        self.mu_layer = nn.Linear(self.hidden_size//2, self.latent_dim)
        self.logvar_layer = nn.Linear(self.hidden_size//2, self.latent_dim)
        
        # CLASSIFIER più semplice
        self.classifier = nn.Sequential(
        ...
        # DECODER più leggero
        self.decoder_layers = nn.Sequential(
            nn.Linear(self.latent_dim, self.hidden_size//2),
            nn.BatchNorm1d(self.hidden_size//2),
            nn.ReLU(inplace=True),
            nn.Dropout(0.2),
...
def knowledge_distillation_loss(student_outputs, teacher_outputs, labels, features, 
                              temperature=4.0, alpha=0.7, beta=0.3):
    """Knowledge Distillation Loss combining soft and hard targets"""
    x_hat_s, mu_s, logvar_s, class_out_s = student_outputs
    x_hat_t, mu_t, logvar_t, class_out_t = teacher_outputs
    
    # Standard CI-VAE loss for student
    student_loss, recon_loss, class_loss, kld_loss = fixed_loss_fn(
        x_hat_s, features, logvar_s, mu_s, class_out_s, labels, alpha=3.0
    )
    
    # Knowledge Distillation loss (soft targets)
    teacher_probs = F.softmax(class_out_t / temperature, dim=1)
    student_log_probs = F.log_softmax(class_out_s / temperature, dim=1)
    ...
    latent_mse_loss = F.mse_loss(mu_s, mu_t.detach())
    ...
    return total_loss, distillation_loss, student_loss, latent_mse_loss
...
def load_all_client_data(user_path='irds_dataset', batch_size=32, num_clients=7):
...
def train_teacher_with_best_model_selection(teacher_model, train_loader, val_loader, device, 
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (features, labels) in enumerate(train_loader):
            ...
            optimizer.zero_grad()
            x_hat, mu, logvar, class_out = teacher_model(features)
...
def train_student_with_best_model_selection(student_model, teacher_model, train_loader, val_loader, 
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (features, labels) in enumerate(train_loader):
            ...
            total_loss, dist_loss, student_loss, latent_loss = knowledge_distillation_loss(
                student_outputs, teacher_outputs, labels, features, temperature=temperature
            )
...
def evaluate_model(model, test_loader, device):
    ...
    with torch.no_grad():
        for features, labels in test_loader:
            ...
            x_hat, mu, logvar, class_out = model(features)
            loss, _, _, _ = fixed_loss_fn(x_hat, features, logvar, mu, class_out, labels)
...
def create_hyperparameter_grid():
    ...
    hyperparameters = {
        'teacher_lr': [0.005, 0.001],
        'student_lr': [0.005, 0.001],
        'teacher_epochs': [200],
        'student_epochs': [120],
        'teacher_gamma': [0.9],
        'student_gamma': [0.9],
...
def run_single_experiment(params, user_path='irds_dataset', num_clients=7, seed=42, 
    ...
    teacher_model = ImprovedCI_VAE_NUMERICAL(
        input_size=1280,
        hidden_size=512,
        latent_dim=7,
        num_classes=4,
    ...
    for client_id in range(num_clients):
        ...
        # Initialize student
        student_model = StudentCI_VAE_NUMERICAL(
            input_size=1280,
            hidden_size=256,
            latent_dim=7,
            num_classes=4,
...
def run_grid_search(user_path='irds_dataset', num_clients=7, seed=42, max_experiments=20):
    """Run grid search over hyperparameters with continuous saving and cross-client evaluation"""
    ...
    results_dir = f"irds_vae_results_clients"
    os.makedirs(results_dir, exist_ok=True)
    ...
    results_filename = f'{results_dir}/kd_vae_cross_client_results_{num_clients}_clients.csv'
...
def run_kd_experiments(user_path='irds_dataset', client_configs=[3, 5, 7], seed=42, 
                      max_experiments_per_config=10):
    """
    Run knowledge distillation experiments for different client configurations
    with comprehensive cross-client analysis for VAE models
    """
    ...
    for num_clients in client_configs:
        print(f"\n{'='*80}")
        print(f"RUNNING VAE KNOWLEDGE DISTILLATION EXPERIMENTS WITH {num_clients} CLIENTS")
        print(f"{'='*80}")
        ...
        print(f"Completed VAE experiments for {num_clients} clients.")
        print(f"Results saved to {filename}")
...
def analyze_cross_client_results(csv_filename, num_clients):
    ...
    print(f"Analyzing VAE results from {csv_filename}")
    print("=" * 60)
    ...
    print(f"Best VAE Experiment (ID: {best_exp['experiment_id']}):")
    ...
    print(f"\nCross-Client Matrix for Best VAE Experiment:")
    print_cross_client_summary_table(matrix, num_clients, 'accuracy')
    
    # Summary statistics
    print(f"\nSummary Statistics across all VAE experiments:")
...
if __name__ == "__main__":
    # Run VAE knowledge distillation experiments for different client configurations
    all_results = run_kd_experiments(
        user_path='/scratch.hpc/mazza/femnist/irds_lab/irds_dataset/',
        client_configs=[7],  # Start with 3 clients for testing
        seed=42,
        max_experiments_per_config=100  # Adjust based on computational budget
    )
    
    print("\nAll VAE knowledge distillation experiments completed!")
    
    # Example of how to analyze results afterwards
    # analyze_cross_client_results('irds_vae_results_3_clients/kd_vae_cross_client_results_3_clients.csv', 3)