...
class TeacherNetwork(nn.Module):
    ...
    def __init__(self, input_size=1280, hidden_sizes=[512, 256, 128], num_classes=10, 
...
class StudentNetwork(nn.Module):
    ...
    def __init__(self, input_size=1280, hidden_sizes=[256, 128], num_classes=10, 
...
def load_validation_data_with_split(user_path='femnist_dataset', batch_size=32, 
                                            num_clients=7, teacher_val_ratio=0.1):
    """
    Load validation data for FEMNIST dataset and split into teacher validation and remaining data.
    For teacher: uses TEST data from all clients, splits into train/val for teacher
    """
    
    all_test_data = []
    all_test_labels = []
    
    # Carica i dati di test da tutti i client per il teacher
    for partition_id in range(num_clients):
        base_path = os.path.join(user_path, f"client_{partition_id}")
        test_feat_path = os.path.join(base_path, "test_features.pt")
        test_label_path = os.path.join(base_path, "test_labels.pt")
        
        # Verifica esistenza file
        for path in [test_feat_path, test_label_path]:
            if not os.path.exists(path):
                raise FileNotFoundError(f"File mancante: {path}")
        
        # Carica dati test del client
        test_features = torch.load(test_feat_path)
        test_labels = torch.load(test_label_path)
        
        all_test_data.append(test_features)
        all_test_labels.append(test_labels)
    
    # Combina tutti i dati di test
    combined_data = torch.cat(all_test_data, dim=0)
    combined_labels = torch.cat(all_test_labels, dim=0)
    
    # Converte le etichette al formato corretto se necessario
    if combined_labels.dtype != torch.long:
...
def load_client_data(partition_id: int, user_path='femnist_dataset', batch_size=32, 
                            student_val_ratio=0.2):
    """
    Load train+test data for a specific FEMNIST client.
    For student: uses TRAIN data, splits into train/val for student training
    """
    ...
    # Verifica esistenza file
    for path in [train_feat_path, train_label_path]:
        if not os.path.exists(path):
            raise FileNotFoundError(f"File mancante: {path}")
    
    # Carica dati di training del client
    ...
    # Split stratificato per dividere i dati di training in train/val per lo student
    X_train, X_val, y_train, y_val = train_test_split(
        train_features, train_labels,
        test_size=student_val_ratio,
        stratify=train_labels,  # Mantiene proporzioni classi
        random_state=42
    )
...
def create_hyperparameter_grid():
    ...
    hyperparameters = {
        'teacher_lr': [0.01, 0.001, 0.005],
        'student_lr': [0.01, 0.001, 0.005],
        'teacher_epochs': [150],
        'student_epochs': [80],
        'teacher_gamma': [0.95],
        'student_gamma': [0.95],
...
def run_grid_search(user_path='femnist_dataset', num_clients=7, seed=42, max_experiments=20):
    ...
    results_filename = f'{results_dir}/kd_cross_client_results_{num_clients}_clients_1.csv'
...
if __name__ == "__main__":
    ...
    all_results = run_kd_experiments(
        user_path='D:/tesi/infocom_poster-main/progetto/femnist_lab/femnist_processed',
        client_configs=[3],  # Start with 3 clients for testing
        seed=42,
        max_experiments_per_config=500  # Reduced for testing
    )
