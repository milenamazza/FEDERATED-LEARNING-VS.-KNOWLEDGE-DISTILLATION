...
ris_path = f"fedmd_results_final_{ds}_7_c.csv"
...
class FedMDSystem:
    ...
    def setup_csv_logging(self):
        """Setup del logging CSV con nuove metriche"""
        self.csv_filename = ris_path

        # Header del CSV esteso con precision, recall, f1 e matrice cross-client
        fieldnames = [
        ...
        # Aggiungi colonne per la matrice cross-client (modello i testato su dataset j)
        # Le righe rappresentano i modelli, le colonne i dataset di test
        for i in range(self.num_clients):
            for j in range(self.num_clients):
                fieldnames.append(f'm{i}{j}')  # modello i testato su dataset j
    ...
    def log_round_results(self, round_num: int, metrics: Dict):
        """Salva le metriche aggregate del round nel CSV con precision, recall, f1 e matrice"""
        with open(self.csv_filename, 'a', newline='') as csvfile:
            fieldnames = [
                'experiment_id', 'round', 'lr', 'temperature', 'consensus_epochs', 
                'local_epochs', 'alpha', 'public_samples', 'batch_size', 'seed',
            ...
            # Aggiungi colonne matrice cross-client
            for i in range(self.num_clients):
                for j in range(self.num_clients):
                    fieldnames.append(f'm{i}{j}')
            ...
            # Nuove metriche
            avg_precision = np.mean([client['precision'] for client in metrics['clients']])
            avg_recall = np.mean([client['recall'] for client in metrics['clients']])
            avg_f1 = np.mean([client['f1'] for client in metrics['clients']])
            ...
            # Aggiungi valori matrice cross-client
            acc_matrix = np.array(metrics['accuracy_matrix'])
            for i in range(self.num_clients):
                for j in range(self.num_clients):
                    row[f'm{i}{j}'] = acc_matrix[i, j]
    ...
    def _evaluate_model_with_metrics(self, model: nn.Module, dataloader: DataLoader) -> Dict:
        """Valuta un modello con accuracy, precision, recall e f1"""
        model.eval()
        all_preds = []
        all_targets = []
        ...
        # Calcola precision, recall, f1 (macro average)
        try:
    ...
    def evaluation_phase(self, round_num: int, distillation_losses: Dict) -> Dict:
        """Fase di valutazione completa con metriche estese"""
        print("📊 Evaluation Phase")
        
        # Matrice di accuracy cross-client
        acc_matrix = np.zeros((self.num_clients, self.num_clients))
        precision_matrix = np.zeros((self.num_clients, self.num_clients))
        recall_matrix = np.zeros((self.num_clients, self.num_clients))
        f1_matrix = np.zeros((self.num_clients, self.num_clients))
        
        for i, client_i in enumerate(self.clients):
            for j in range(self.num_clients):
                _, test_loader = load_data(j, self.num_clients, batch_size=self.config['batch_size'])
                metrics = self._evaluate_model_with_metrics(client_i['model'], test_loader)
                
                acc_matrix[i, j] = metrics['accuracy']
                precision_matrix[i, j] = metrics['precision']
                recall_matrix[i, j] = metrics['recall']
                f1_matrix[i, j] = metrics['f1']
        
        # Calcola metriche per client
        client_metrics = []
        for i, client in enumerate(self.clients):
            self_acc = acc_matrix[i, i]
            self_precision = precision_matrix[i, i]
            self_recall = recall_matrix[i, i]
            self_f1 = f1_matrix[i, i]
            
            cross_accs = [acc_matrix[i, j] for j in range(self.num_clients) if j != i]
            cross_precisions = [precision_matrix[i, j] for j in range(self.num_clients) if j != i]
            cross_recalls = [recall_matrix[i, j] for j in range(self.num_clients) if j != i]
            cross_f1s = [f1_matrix[i, j] for j in range(self.num_clients) if j != i]
            
            avg_cross_acc = np.mean(cross_accs) if cross_accs else 0.0
            avg_cross_precision = np.mean(cross_precisions) if cross_precisions else 0.0
            avg_cross_recall = np.mean(cross_recalls) if cross_recalls else 0.0
            avg_cross_f1 = np.mean(cross_f1s) if cross_f1s else 0.0
        ...
        print("\n📋 Cross-Client Accuracy Matrix (Rows=Models, Columns=Test_Datasets):")
        header = "         " + "  ".join([f"D{j}" for j in range(self.num_clients)])
        print(header)
        for i in range(self.num_clients):
        ...
        avg_precision = np.mean([precision_matrix[i, i] for i in range(self.num_clients)])
        avg_recall = np.mean([recall_matrix[i, i] for i in range(self.num_clients)])
        avg_f1 = np.mean([f1_matrix[i, i] for i in range(self.num_clients)])
    ...
    def _evaluate_model(self, model: nn.Module, dataloader: DataLoader) -> float:
        """Valuta accuracy di un modello (mantenuto per compatibilità)"""
        metrics = self._evaluate_model_with_metrics(model, dataloader)
...
def analyze_results(csv_file: str) -> None:
    ...
    print(f"Avg self accuracy: {df['avg_self_accuracy'].mean()*100:.2f}%")
    print(f"Avg cross accuracy: {df['avg_cross_accuracy'].mean()*100:.2f}%")
    print(f"Avg precision: {df['avg_precision'].mean():.3f}")
    print(f"Avg recall: {df['avg_recall'].mean():.3f}")
    print(f"Avg F1: {df['avg_f1'].mean():.3f}")
    ...
    with open(report_file, 'w') as f:
        ...
        f.write(f"Avg self accuracy: {df['avg_self_accuracy'].mean()*100:.2f}%\n")
        f.write(f"Avg cross accuracy: {df['avg_cross_accuracy'].mean()*100:.2f}%\n")
        f.write(f"Avg precision: {df['avg_precision'].mean():.3f}\n")
        f.write(f"Avg recall: {df['avg_recall'].mean():.3f}\n")
        f.write(f"Avg F1: {df['avg_f1'].mean():.3f}\n\n")
