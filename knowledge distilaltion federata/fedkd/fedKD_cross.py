...
class FedKDServer:
    ...
    def evaluate_global_model(self, test_loaders: List[DataLoader]) -> Dict:
        """Evaluate global model on all clients with detailed metrics"""
        self.global_model.eval()
        
        client_accuracies = []
        client_f1_scores = []
        client_precision_scores = []
        ...
        with torch.no_grad():
            for client_id, test_loader in enumerate(test_loaders):
                ...
                for x, y in test_loader:
                    ...
                    client_true.extend(y.cpu().numpy())
                    client_pred.extend(predicted.cpu().numpy())
                ...
                if len(client_true) > 0:
                    # Use 'weighted' average for multiclass, handles class imbalance
                    client_f1 = f1_score(client_true, client_pred, average='weighted', zero_division=0)
                    client_precision = precision_score(client_true, client_pred, average='weighted', zero_division=0)
                    client_recall = recall_score(client_true, client_pred, average='weighted', zero_division=0)
                else:
                    client_f1 = client_precision = client_recall = 0.0
                
                client_f1_scores.append(client_f1)
                client_precision_scores.append(client_precision)
                client_recall_scores.append(client_recall)
        
        global_accuracy = total_correct / total_samples if total_samples > 0 else 0
        
        # Calculate global metrics
        if len(all_true_labels) > 0:
            global_f1 = f1_score(all_true_labels, all_predicted_labels, average='weighted', zero_division=0)
            global_precision = precision_score(all_true_labels, all_predicted_labels, average='weighted', zero_division=0)
            global_recall = recall_score(all_true_labels, all_predicted_labels, average='weighted', zero_division=0)
        else:
            global_f1 = global_precision = global_recall = 0.0
        ...
        return {
            ...
            'client_precision_scores': client_precision_scores,
            'client_recall_scores': client_recall_scores,
            'avg_client_accuracy': np.mean(client_accuracies) if client_accuracies else 0,
            'std_client_accuracy': np.std(client_accuracies) if client_accuracies else 0,
            'avg_client_f1': np.mean(client_f1_scores) if client_f1_scores else 0,
            'avg_client_precision': np.mean(client_precision_scores) if client_precision_scores else 0,
            'avg_client_recall': np.mean(client_recall_scores) if client_recall_scores else 0
        }
...
def run_federated_knowledge_distillation(num_clients: int = 7,
    ...
    for round_num in range(num_rounds):
        ...
        round_metrics = {
            ...
            'global_f1': eval_results['global_f1'],
            'global_precision': eval_results['global_precision'],
            'global_recall': eval_results['global_recall'],
            'avg_client_accuracy': eval_results['avg_client_accuracy'],
            'std_client_accuracy': eval_results['std_client_accuracy'],
            'avg_client_f1': eval_results['avg_client_f1'],
            'avg_client_precision': eval_results['avg_client_precision'],
            'avg_client_recall': eval_results['avg_client_recall'],
            'client_accuracies': eval_results['client_accuracies'],
            'client_f1_scores': eval_results['client_f1_scores'],
            'client_precision_scores': eval_results['client_precision_scores'],
            'client_recall_scores': eval_results['client_recall_scores'],
...
def log_fedkd_results_to_csv(filepath: str, results: Dict):
    """Log FedKD results to CSV file with early stopping info and enhanced metrics"""
    import csv
    
    file_exists = os.path.isfile(filepath)
    
    # Determine the number of clients from the first performance record
    num_clients = results['num_clients']
    
    # Build fieldnames dynamically based on number of clients
    base_fieldnames = [
        'algorithm', 'dataset', 'num_clients', 'num_rounds',
        ...
        'round', 'global_accuracy', 'global_f1', 'global_precision', 'global_recall',
    ...
    for i in range(num_clients):
        client_fieldnames.extend([
            f'client_{i}_accuracy',
            f'client_{i}_f1', 
            f'client_{i}_precision',
            f'client_{i}_recall'
        ])
    
    fieldnames = base_fieldnames + client_fieldnames
    ...
    with open(filepath, mode='a', newline='') as csv_file:
        ...
        for perf in results['performance_history']:
            row = {
                ...
                'global_f1': perf['global_f1'],
                'global_precision': perf['global_precision'],
                'global_recall': perf['global_recall'],
            ...
            # 🔑 aggiungi metriche per-client
            for i in range(num_clients):
                row[f'client_{i}_accuracy'] = perf['client_accuracies'][i]
                row[f'client_{i}_f1'] = perf.get('client_f1_scores', [None]*num_clients)[i]
                row[f'client_{i}_precision'] = perf.get('client_precision_scores', [None]*num_clients)[i]
                row[f'client_{i}_recall'] = perf.get('client_recall_scores', [None]*num_clients)[i]
...
if __name__ == "__main__":
    ...
    patience = 50  # Wait 5 rounds without improvement
